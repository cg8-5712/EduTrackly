...
export function formatDatefromyyyymmddtopsqldate(date) {
  if (typeof date !== 'string' || date.length !== 8) {
    logger.error('Invalid date format for YYYYMMDD conversion', {
      providedDate: date,
      expectedFormat: 'YYYYMMDD',
      error: FormatErrors.NOT_YYYYMMDD_DATE
    });
...
export function formatDatefromsqldatetoyyyymmdd(date) {
  ...
  if (isNaN(utcDate.getTime())) {
    logger.error('Invalid date format for SQL to YYYYMMDD conversion', {
      providedDate: date,
      error: FormatErrors.NOT_YYYYMMDDHHMMSS_DATE
    });
...
export function formatDateFromSqlTimestampToTimestamp(sqlTimestamp) {
  ...
  try {
    ...
    logger.error('formatDateFromSqlTimestampToTimestamp error', {
      error: {
        message: err.message,
        name: err.name,
        stack: err.stack
